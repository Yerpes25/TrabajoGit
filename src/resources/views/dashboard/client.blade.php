        ...
        <div style="max-width: 1280px; margin: 0 auto; padding: 0 1.5rem;">
                        ...
                        <div class="flex-1 w-full flex flex-col items-center justify-center md:border-r border-slate-100 md:pr-8">
                            ...
                            <div class="flex flex-col items-center gap-1.5">
                                <div class="flex items-baseline gap-1">
                                    <span class="text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $horas }}</span>
                                    <span class="text-2xl text-slate-400 font-bold mr-2">h</span>

                                    <span class="text-6xl font-black text-slate-900 tracking-tighter leading-none">{{ $minutos }}</span>
                                ...
                                <span class="bg-slate-50 text-slate-600 text-[11px] font-bold px-3 py-1.5 rounded-xl border border-slate-200 inline-flex items-center gap-1.5 shadow-sm">
                                    ...
                                    Formato exacto: {{ gmdate('H:i:s', $balanceSeconds) }}
                                </span>
                            </div>
                        ...
                        <div class="flex-1 w-full flex flex-col justify-center items-center h-full md:pl-4">
                            ...
                            <div class="w-full max-w-sm bg-gradient-to-br from-[#62bd19]/5 to-transparent border border-[#62bd19]/20 p-6 rounded-2xl flex items-center gap-6 transition-colors hover:border-[#62bd19]/40">
                                ...
                                <div>
                                    ...
                                    <span class="text-4xl font-black text-[#62bd19] block leading-none">{{ $validated }}</span>
                                </div>
                            </div>
            ...
            <div x-data="{ showAll: false }" class="mt-16">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    @foreach($activeBonuses as $index => $bonus)
                    @php
                    ...
                    <div
                        ...
                        {{-- AQUÍ ELIMINAMOS: hover:-translate-y-2 y redujimos duration-500 a duration-200 --}}
                        class="bg-white rounded-[2rem] border {{ $isPopular ? 'border-[#62bd19]' : 'border-slate-100' }} p-10 flex flex-col items-center text-center relative group hover:shadow-xl transition-shadow duration-200"
                    >
                        @if($isPopular)
                        ...
                        {{-- AQUÍ ELIMINAMOS: group-hover:rotate-12 transition-transform duration-300 --}}
                        <div class="w-20 h-20 {{ $isPopular ? 'bg-[#f4faed] text-[#62bd19]' : 'bg-slate-50 text-slate-400' }} rounded-3xl flex items-center justify-center mb-6">
                            <span class="material-symbols-outlined text-4xl">{{ $icon }}</span>
                        </div>

                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2">{{ $bonus->name }}</span>
                        <h4 class="text-3xl font-black text-slate-900 mb-8">{{ $horas }} <span class="text-lg text-slate-400 font-medium">Horas</span></h4>

                        <button class="w-full py-4 px-6 rounded-2xl font-black transition-colors duration-200 {{ $isPopular ? 'bg-[#62bd19] text-white hover:bg-[#52a115] shadow-lg shadow-green-100' : 'bg-white text-slate-900 border-2 border-slate-900 hover:bg-slate-900 hover:text-white' }}">
                            Seleccionar
                        </button>
                    </div>
                    @endforeach
                ...
                {{-- Botón VER TODOS Centrado y limpio --}}
